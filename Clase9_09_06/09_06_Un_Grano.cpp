//Septiembre 06 2022 (Clase 9)
// Modelar el movimiento de Un Grano 2D con la Fuerza de Hertz-Kuwabara-Kono
//Programa Original: Gas2D
#include <iostream>
#include <cmath>
#include "../vector.h"
#include "../Random64.h"
...
const double g=98, Gamma=80; //Gravedad Alta, para verificar velocidad de animación
...
void InicieAnimacion(void){
  ...
  cout<<"set output '09_06_Un_Grano.gif'"<<endl;
  cout<<"unset key"<<endl;
...
int main(void){
  ...
  int i,ix,iy;
  double t,tdibujo,dt=1e-3,tmax=3*sqrt(Ly/g),tcuadro=tmax/50;
  ...
  for(ix=0;ix<Nx;ix++)
    for(iy=0;iy<Ny;iy++){
      ...
      //-----------------------(   x0,   y0,          Vx0,          Vy0, theta0,omega0  ,m0,R0)
      Molecula[Nx*iy+ix].Inicie((ix+1)*dx,(iy+1)*dy,    0,            0,      0,OmegaMax,m0,R0);//Iniciamos a mano las velocidades
    }
